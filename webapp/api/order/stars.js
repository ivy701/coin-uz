...
function callFragmentApi(endpoint, body) {
  return new Promise((resolve) => {
    const apiKey = (process.env.FRAGMENT_API_KEY || '').trim();
    if (!apiKey) {
      return resolve({ ok: false, error: 'FRAGMENT_API_KEY sozlanmagan' });
    }
...
module.exports = async (req, res) => {
  ...
  // 3. Rate limiting (max 5 requests per minute)
